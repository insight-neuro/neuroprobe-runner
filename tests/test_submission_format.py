...
class TestSubmissionFormat:
    ...
    def test_split_directories_exist(self, submission_dirs):
        ...
        for submission_dir in submission_dirs:
            existing_splits = {
                ...
                if d.is_dir() and d.name in valid_splits
            }
        ...
        for submission_dir in submission_dirs:
            ...
            for split_dir in split_dirs:
                ...
                population_files = [
                    ...
                    if f.name.startswith("population_") and f.name.endswith(".json")
                ]

                # Extract task names from filenames
                found_tasks = {
                    f.name.replace("population_", "").replace(".json", "")
    ...
    def test_publication_bib_format(self, submission_dirs):
        ...
        for submission_dir in submission_dirs:
            ...
            assert content, f"PUBLICATION.bib cannot be empty in {submission_dir.name}"
