...
parser = argparse.ArgumentParser(
    description="Create performance figure for BTBench evaluation"
)
parser.add_argument(
    "--split_type",
    type=str,
...
def parse_results_default(model):
    for task in task_name_mapping.keys():
        ...
        for subject_id, trial_id in subject_trials:
            filename = (
                model["eval_results_path"]
                + f"population_btbank{subject_id}_{trial_id}_{task}.json"
            )
            ...
            try:
                ...
                try:
                    ...
                    print(
                        f"Warning: Still invalid JSON in file {filename} after removing last character: {e2}"
                    )
                    continue

            if (
                "one_second_after_onset"
                in data["evaluation_results"][f"btbank{subject_id}_{trial_id}"][
                    "population"
                ]
            ):  # XXX remove this later, have a unified interface for all models
                data = data["evaluation_results"][f"btbank{subject_id}_{trial_id}"][
                    "population"
                ]["one_second_after_onset"]
            else:
                data = data["evaluation_results"][f"btbank{subject_id}_{trial_id}"][
                    "population"
                ]["whole_window"]  # for BrainBERT only
            value = np.nanmean(
                [fold_result["test_roc_auc"] for fold_result in data["folds"]]
            )
...
for model in models:
    means = [
        performance_data[task][model["name"]]["mean"]
        for task in task_name_mapping.keys()
    ]
    sems = [
        performance_data[task][model["name"]]["sem"]
        for task in task_name_mapping.keys()
    ]
...
for model in models:
    model["color"] = sns.color_palette(
        model["color_palette"], color_palette_ids[model["color_palette"]]
    )[model["color_palette_id"]]
...
gs = gridspec.GridSpec(
    n_rows, n_cols, height_ratios=height_ratios, hspace=0.3, wspace=0.2
)
...
legend_ax.legend(
    handles,
    [model["name"] for model in models] + ["Chance"],
    loc="center left",
